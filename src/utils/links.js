export const SERVICES_LINK = '/#services';
export const COMPANY_LINK = '/';
export const PORTFOLIO_LINK = '/#portfolio';
export const ABOUT_LINK = '/about-us';
export const CONTACTUS_LINK = '/contact-us';
export const BLOGS_LINK = '/blog';

export const FAQ_LINK = '/faq';
export const PRIVACY_POLICY_LINK = '/privacy';
export const TERMS_LINK = '/terms';
export const Contributors ='https://www.freepik.com/free-vector/business-man-working-hard-stock-financial-trade-market-diagram-vector-illustration-flat-design_13399753.htm#&position=30&from_view=author'

export const WEB_DEV_SERVICE_LINK = '/service/web-development';
export const MOBILE_DEV_SERVICE_LINK = '/service/magento-development';
export const UI_UX_LINK = '/service/seo-services';
